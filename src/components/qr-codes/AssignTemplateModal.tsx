...
export const AssignTemplateModal: React.FC<AssignTemplateModalProps> = ({
  ...
  useEffect(() => {
    if (qrCode) {
      setSelectedTemplateId(qrCode.templateId || '');
    }
  }, [qrCode]);
      ...
      <div className="bg-background w-full max-w-md rounded-xl shadow-2xl border border-border animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-semibold text-foreground">Assign Template</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground pt-1">
              Update the visual appearance of QR Code #{qrCode.numericId}.
            </p>
          </div>
